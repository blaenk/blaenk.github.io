...
I still created my own implementation as a Pandoc AST for a few reasons. The first is that Pandoc's generated table of contents are plain unordered lists, as was output by kramdown. This would mean that to achieve the fancy numbering scheme I use I'd have to implement it in JavaScript. Furthermore, I wouldn't have access to the table of contents field in the post itself. This would mean that I wouldn't be able to selectively show a table of contents based on the post -- though that can be implemented in metadata, i.e. `toc: true` -- or more importantly to me, choose the location at which it should appear, i.e. after an introductory paragraph.

However, I will consider these supposed downsides and weigh them against some potential upsides, mainly keeping things DRY. I feel that I would much rather use the hopefully tried and proven table of contents generation code from Pandoc than re-invent my own. I'll update this post if I do end up adopting such a scheme.
